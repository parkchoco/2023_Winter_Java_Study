...
public class all {//좌우 바꾸는 형태
	...
	all(String U_InputFile_Path, String U_OutputFile_Path) {
		...
		for(column =2;column<height-2;column++) {
			...
			for (row = 2; row<width-2;row++) {
				...
				for(int i=-2;i<=2;i++) {
					for(int j=-2;j<=2;j++) {
						color = new Color(SourceImage.getRGB(row+i, column+j));//row, column의 특정한 위치에 있는 색깔을 읽어와라
						red+=(int)(color.getRed());//레드가 뜯어져나온다->분해한다
						green+=(int)(color.getGreen());
						blue+=(int)(color.getBlue());
					}
				...
				new_color = new Color(red/25,green/25,blue/25);//세가지 색을 썪어서 색만들기
			
				for(int i=-2;i<=2;i++) {
					for(int j=-2;j<=2;j++) {
						TargetImage.setRGB(row/2+width/2+i/2, column/2+j/2, new_color.getRGB());
					}
				}
			}
		}
		...
		for(column =1;column<height-1;column++) {
				...
				for (row = 1; row<width-1;row++) {
					...
					color = new Color(SourceImage.getRGB(row-1/2, column));
					...
					new_color = new Color(new_red,new_green,new_blue);//세가지 색을 썪어서 색만들기
					
					
					for(int i=-1;i<=1;i++) {
						for(int j=-1;j<=1;j++) {
							TargetImage.setRGB(row/2+i/2, column/2+height/2+j/2, new_color.getRGB());
						}
					}
				}
			}
	for(column =1;column<height-1;column++) {
		...
		for(int i=-1;i<=1;i++) {
			for(int j=-1;j<=1;j++) {
				TargetImage.setRGB(row/2+width/2+i/2, column/2+height/2+j/2, new_color.getRGB());
			}
		}
	}
}
