...
public class FlipHorizontalImage {//좌우 바꾸는 형태
	...
	FlipHorizontalImage(String U_InputFile_Path, String U_OutputFile_Path) {
		...
		for(column =0;column<=height-1;column++) {
			for (row = 0; row<=width-1;row++) {
				...
				TargetImage.setRGB((width-1-row)/2+width/2, column/2, color.getRGB());
				TargetImage.setRGB(row/2, (height-1-column)/2+height/2, color.getRGB());
				TargetImage.setRGB((width-1-row)/2+width/2, (height-1-column)/2+height/2, color.getRGB());
