...
public class Colorfull {//좌우 바꾸는 형태
	...
	Colorfull(String U_InputFile_Path, String U_OutputFile_Path) {
		...
		for(column =0;column<=height-1;column++) {
			for (row = 0; row<=width-1;row++) {
				...
				new_color = new Color(red,0,0);//세가지 색을 썪어서 색만들기
				TargetImage.setRGB(row/2+width/2, column/2, new_color.getRGB());
				
				green=(int)(color.getGreen());//레드가 뜯어져나온다->분해한다
				new_color = new Color(0,green,0);//세가지 색을 썪어서 색만들기
				TargetImage.setRGB(row/2, column/2+height/2, new_color.getRGB());
				
				blue=(int)(color.getBlue());//레드가 뜯어져나온다->분해한다
				new_color = new Color(0,0,blue);//세가지 색을 썪어서 색만들기
