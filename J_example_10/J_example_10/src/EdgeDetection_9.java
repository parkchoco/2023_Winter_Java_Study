...
public class EdgeDetection_9 {//좌우 바꾸는 형태
	...
	EdgeDetection_9(String U_InputFile_Path, String U_OutputFile_Path) {
		...
		for(column =0;column<=height-1;column++) {
			...
			for (row = 0; row<=width-1;row++) {
				...
				if(((red+green+blue)/3-(new_red+new_green+new_blue)/3)>9)
